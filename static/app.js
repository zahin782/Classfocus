class ClassFocusApp {
    ...
    processAnalysisResult(result) {
        ...
        if (intensity === 'none') {
            // For neutral emotion or no intensity
            emotionText = `${emotionCapitalized} — ${probPercent}%`;
        } else {
            emotionText = `${emotionCapitalized} (${intensity}) — ${probPercent}%`;
        }
    ...
    drawFaceBox(faceBox) {
        ...
        if (faceBox) {
            ...
            ctx.strokeStyle = '#00ff41';  // Green color like phone cameras
            ctx.lineWidth = 3;
            ctx.setLineDash([]);
